...
class Internal(Node):
    ...
    def add_child(
        self, data: Union[int, list], go_left: bool
    ) -> Union["Internal", Leaf]:
...
def testcase():
    sailFish_numbers = (
        open(os.path.join(os.path.dirname(__file__), "day-18-test.txt"))
        .read()
        .split("\n")
    )
...
if __name__ == "__main__":
    ...
    sailFish_numbers = (
        open(os.path.join(os.path.dirname(__file__), "day-18-input.txt"))
        .read()
        .split("\n")
    )
