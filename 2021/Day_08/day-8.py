...
def part2(data):
    ...
    for instruction in data:
        ...
        for sig in signal:
            seven_bit_set[check_set.get(len(sig), -1)] = (
                set(sig) if len(sig) in check_set else None
            )
...
def testcase():
    signal_output = (
        open(os.path.join(os.path.dirname(__file__), "day-8-test.txt"))
        .read()
        .split("\n")
    )
...
if __name__ == "__main__":
    ...
    signal_output = (
        open(os.path.join(os.path.dirname(__file__), "day-8-input.txt"))
        .read()
        .split("\n")
    )
