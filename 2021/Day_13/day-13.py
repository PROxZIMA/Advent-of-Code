...
def both_part(data):
    ...
    max_x, max_y = max(dots, key=lambda x: int(x[0])), max(
        dots, key=lambda x: int(x[1])
    )
...
def testcase():
    random_dots = open(
        os.path.join(os.path.dirname(__file__), "day-13-test.txt")
    ).read()
...
if __name__ == "__main__":
    ...
    random_dots = open(
        os.path.join(os.path.dirname(__file__), "day-13-input.txt")
    ).read()
    random_dots = random_dots.split("\n")
